class Api::V1::RsdProductsController < ApiController
  ...
  def index
    rsd_products = RsdProduct.filter(params[:filter], params[:receipt_accepted_count], params[:unique_rejected_count], params[:unique_accepted_count]).order("rsd ASC").page(params[:page]).per(params[:limit])
